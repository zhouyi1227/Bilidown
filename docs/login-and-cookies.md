...
Bilidown 不接收账号密码，也不会绕过 Bilibili 权限。推荐使用应用内二维码登录：应用向 Bilibili 请求一次性二维码，在本地显示并轮询确认状态，只把必要的 Bilibili Cookie 保存在当前运行的后端内存中。

## 扫码登录（推荐）

1. 点击“扫码登录 Bilibili”。
2. 使用已登录的 Bilibili 手机客户端扫码，并在手机上确认。
3. 主窗口检测到确认后自动建立内存登录态；Cookie 仅在本次 Bilidown 运行中有效，退出后清除。

二维码密钥与轮询请求仅在 Bilidown 和 Bilibili 之间传输；二维码图像在本地生成。这个流程不依赖 Chrome/Edge Cookie 数据库，因此不会遇到应用绑定加密问题；Bilidown 不读取或保存你的密码。
...
Chrome/Edge 在 Windows 上通常会因 DPAPI 或应用绑定加密无法直接解密。这不是用户操作错误。请优先使用应用内扫码登录；Firefox 直读或 `cookies.txt` 只作为高级兼容路径。
