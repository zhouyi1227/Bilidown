...
先查看系统托盘；关闭窗口后 Bilidown 默认仍在后台。Windows 可双击托盘图标或选择“打开 Bilidown”，macOS 也可点击 Dock 中仍在运行的 Bilidown 图标。仍无窗口时再检查任务管理器/活动监视器和本地环回防火墙。程序只支持 `127.0.0.1`，不能从手机或局域网访问。

如果窗口显示“本地后端启动失败”，可直接选择“重试启动”，无需强制结束整个应用。重复失败时检查下方两份启动日志；日志只记录在本机。
...
桌面壳检测到 sidecar 缺失、提前退出或启动超时时，还会写入同目录的 `desktop-startup-error.log`。该文件仅包含脱敏错误类别，不包含会话令牌、Cookie 或 URL。
